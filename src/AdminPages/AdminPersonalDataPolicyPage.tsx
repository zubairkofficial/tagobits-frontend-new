...
interface PersonalDataPolicyData {
  id: string;
  content: string;
  created_at: string;
  updated_at: string;
...
interface ApiResponse {
  success: boolean;
  data: PersonalDataPolicyData;
  detail?: string;
  knowledge_base?: string;
  knowledge_base_error?: string;
}

const AdminPersonalDataPolicyPage: React.FC = () => {
  ...
  const createDefaultContent = () => {
    ...
    return `<h1>Personal Data Policy</h1>
<p><strong>Last updated:</strong> ${defaultDate}</p>
<p>At TagoCash, we are committed to protecting your personal data and ensuring it is handled in accordance with applicable data protection laws.</p>
<h2>Types of Personal Data We Process</h2>
<p>We may collect and process the following personal data:</p>
<ul>
  <li>Identity information (name, date of birth, national identification)</li>
  ...
  <li>Transaction data (records of payments, transfers, and other financial activities)</li>
...
<p>We retain your personal data for as long as necessary to fulfill the purposes for which we collected it, including for the purposes of satisfying any legal, regulatory, tax, accounting, or reporting requirements.</p>`;
  };
  ...
  const editor = useEditor({
    extensions: [
      ...
      Placeholder.configure({
        placeholder: 'Enter Personal Data Policy content here...',
      }),
    ],
    content: '',
    onUpdate: ({ editor }) => {
  ...
  const fetchPolicy = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get<ApiResponse>('/personal-data-policy');
      if (response.data.success) {
        ...
        setError('Failed to load Personal Data Policy');
      }
    } catch (err: any) {
      if (err.response && err.response.status === 404) {
        // Policy not found - show a message and allow creation
        setError('No Personal Data Policy found. You can create a new one.');
        const defaultContent = createDefaultContent();
        setContent(defaultContent);
        if (editor) {
          editor.commands.setContent(defaultContent);
        ...
        setError('Failed to load Personal Data Policy. Please try again later.');
        console.error('Error fetching personal data policy:', err);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async () => {
    if (!content.trim()) {
      toast.error('Personal Data Policy cannot be empty');
      return;
    }
    ...
    try {
      ...
      const endpoint = '/personal-data-policy';
      ...
      if (response.data.success) {
        ...
        let successMessage = isCreating ? 'Personal Data Policy created successfully' : 'Personal Data Policy updated successfully';
        ...
        toast.error(response.data.detail || 'Failed to update Personal Data Policy');
      }
    } catch (err: any) {
      console.error('Error updating personal data policy:', err);
      toast.error(err.response?.data?.detail || 'An error occurred while updating Personal Data Policy');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">
          {isNewPolicy ? 'Create Personal Data Policy' : 'Manage Personal Data Policy'}
        </h1>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-700">
              {isNewPolicy ? 'Create New Personal Data Policy' : 'Edit Personal Data Policy'}
            </h2>
            {!isNewPolicy && (
...
export default AdminPersonalDataPolicyPage; 