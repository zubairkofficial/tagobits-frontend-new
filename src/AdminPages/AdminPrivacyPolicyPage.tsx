...
const AdminPrivacyPolicyPage: React.FC = () => {
...
<p>At TagoCash, we respect your privacy and are committed to protecting your personal data. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our services.</p>
...
<p>We implement appropriate security measures to protect your personal information from unauthorized access, alteration, disclosure, or destruction. However, no method of transmission over the Internet or electronic storage is 100% secure, and we cannot guarantee absolute security.</p>`;
  ...
  const fetchPolicy = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get<ApiResponse>('/privacy-policy');
      ...
      if (err.response && err.response.status === 404) {
        ...
        setError('No Privacy Policy found. You can create a new one.');
        const defaultContent = createDefaultContent();
        setContent(defaultContent);
        if (editor) {
          editor.commands.setContent(defaultContent);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        let successMessage = isCreating ? 'Privacy Policy created successfully' : 'Privacy Policy updated successfully';
        ...
        toast.error(response.data.detail || 'Failed to update Privacy Policy');
      }
    } catch (err: any) {
      console.error('Error updating privacy policy:', err);
      toast.error(err.response?.data?.detail || 'An error occurred while updating Privacy Policy');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">
          {isNewPolicy ? 'Create Privacy Policy' : 'Manage Privacy Policy'}
        </h1>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-4">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-700">
              {isNewPolicy ? 'Create New Privacy Policy' : 'Edit Privacy Policy'}
            </h2>
            {!isNewPolicy && (
